...
class TodayViewController: UIViewController, NCWidgetProviding {
    ...
    func renderData(sData: Array<LMSchedule> , isA: Bool , srData: LMWidgetSunRun, elecData: LMWidgetElec, cardData: LMWidgetCard) {
        let allVc = UIHostingController(rootView:
            AllInwidget(sData: sData, availabe: isA, cardData: cardData, srData: srData, elecData: elecData)
            { cb in
    ...
    override func viewDidLoad() {
        ...
        //                // Put your code which should be executed with a delay here
            self.renderData(sData: self.sData, isA: self.Av, srData: self.srData, elecData: self.elecData, cardData: self.cardData)
            })
    }
    func widgetActiveDisplayModeDidChange(_ activeDisplayMode: NCWidgetDisplayMode, withMaximumSize maxSize: CGSize) {
        if activeDisplayMode == .expanded {
            self.preferredContentSize = CGSize(width: maxSize.width, height: 380)
        }else {
